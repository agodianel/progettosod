...
INFLUXDB_DB="ameba"  # Replace "ameba" with your InfluxDB database name
...
write_to_influxdb() {
    ...
    if [[ "${#values[@]}" -eq 5 ]]; then
        ...
        # Format the data as InfluxDB line protocol with measurement name "data"
...
mosquitto_sub -h "$MQTT_BROKER" -t "$MQTT_TOPIC" | while read -r line; do
